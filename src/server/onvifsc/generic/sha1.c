...
void SHA1Input(SHA1_CTX* context, unsigned char *_data, unsigned long len)   
{   
        unsigned long i, j;   
...
void SHA1Result(SHA1_CTX *context, unsigned char *digest)   
{   
        unsigned long i;   
        ...
        SHA1Input(context, (unsigned char *) "\200", 1);   
        while ((context->count[0] & 504) != 448) {   
                SHA1Input(context, (unsigned char *) "\0", 1);   
        }   
 ...
 ***************************************************************************/   
void SHA1_Perform(unsigned char *indata, unsigned long inlen, unsigned char *outdata) //¼ÆËãSHA-1µÄAPI   
{   
    SHA1_CTX sha;   
    SHA1Reset(&sha);   
...
{   
    ...
    memcpy(inputstr, "a", 1);   
    for(i = 0; i < 1000000L; ++i)  //¼ÆËã1000000¸ö¡°a¡±µÄ¹þÏ£Öµ   
    {   
