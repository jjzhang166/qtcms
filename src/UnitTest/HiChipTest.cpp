...
//1,不注册事件DeviceFound，调用start（）开始搜索，通过回调函数取得							｜start返回0，取不到数据，stop返回0,不能取得数据
//查询信息与设置好的数据对比，查看是否一致。调用stop结束。  
//2,注册事件DeviceFound，调用start（）开始搜索，通过回调函数取得							｜start返回0，回调取得的数据与设置的数据一致，stop返回0,不能取得数据
//查询信息与设置好的数据对比，查看是否一致。 调用stop结束。 
//3,注册事件DeviceFound，调用start（）开始搜索，再次调用start（）通							｜start第一次返回0，取得的查询信息与设置的一致，第二次返回-1，stop返回0,不能取得数据
//过回调函数取得查询信息与设置好的数据对比，查看是否一致。调用stop结束。
//4,注册事件DeviceFound，调用start（）开始搜索，调用start（），再							｜start返回0，取得的查询信息与设置的一致，stop第一次返回0，第二次返回-1,不能取得数据
//次调用stop（）通过回调函数取得查询信息与设置好的数据对比，查看是否一致。
//5,注册事件DeviceFound，调用start（）开始搜索，调用flush（）刷新，							｜start返回0，flush返回0，回调取得的数据与设置的数据一致，stop返回0,不能取得数据
//通过回调函数取得查询信息与设置好的数据对比，查看是否一致。调用stop结束。 
//6,注册事件DeviceFound，调用start（）开始搜索，记录收到数据的时间T1。						｜start返回OK，setInterval返回OK，T2>T1且T2=30秒
//调用setInterval（）设置时间间隔为30S，记录收到数据的时间T2，比较T1,T2。


int __cdecl OUTPUT(QString evname,QVariantMap info,void *puser)
{
	if ( "DeviceFound" == evname )
	{
