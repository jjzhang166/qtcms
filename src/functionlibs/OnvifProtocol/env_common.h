...
{
	IN enNVP_EVENT type; // required
	IN int index;
	bool status;
}stNVP_EVENT, *lpNVP_EVENT;

typedef void (*fNVPEventHook)(int event, unsigned int lparam, unsigned int rparam, void *custom /* top-level-param */);
...
typedef struct NVP_SUBSCRIBE
{
	...
	char reference[NVP_MAX_URI_SIZE]; // peer reference
