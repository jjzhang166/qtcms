...
void div2_2::setSubWindows( QWidget * windows,int count )
{
...
void div2_2::flush()
{
	...
	if(!m_bIsMax)  //当没有子窗口被放大时
	{
		...
		for(int j = 0;j < m_nSubWindowCount;j++)       //  总的子窗口数目
		{
			if (j >= i && j < i + m_nRow * m_nColumn)
			{
				...
				{
				case 0:
					m_subWindows[j].setGeometry(0,0,nWidth,nHeight);
					break;
				case 1:
					m_subWindows[j].setGeometry(nWidth,0,nWidth,nHeight);
					break;
				case 2:
					m_subWindows[j].setGeometry(0 ,nHeight,nWidth,nHeight);
					break;
				case 3:
					m_subWindows[j].setGeometry( nWidth, nHeight,nWidth,nHeight);
					break;
				default:
					break;
				}
	...
	{
		m_subWindows[m_nCurrentPage].setGeometry(0     ,
			                                     0     ,
			                                     m_parentOfSubWindows->width()  ,
												 m_parentOfSubWindows->height()
												 );
		m_subWindows[m_nCurrentPage].show();
	...
	else
	{
		...
		for(int j = 0;j < m_nSubWindowCount;j++)
		{
			if (j >= i && j < i + m_nRow * m_nColumn )
			{
				...
				m_subWindows[j].setGeometry(nPositionX,nPositionY,nWidth,nHeight);			
				m_subWindows[j].show();
			}
			else
				m_subWindows[j].hide();
