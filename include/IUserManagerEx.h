...
{
	virtual int addUser(const QString &sUserName,const QString &sPassword,quint64 uiLimit,QVariantMap tSubCode)=0;//0:添加用户成功；1：添加用户失败
	...
	virtual int modifyUserInfo(const QString &sOldUserName,const QString &sNewUserName,const QString &sNewPassword,quint64 uiLimit, QVariantMap tSubCode)=0;//0:设置成功，1：设置失败
};
