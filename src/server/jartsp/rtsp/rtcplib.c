...
static inline int encode_int32be(char *output, unsigned int nVal) 
{
    output[3] = nVal;
    nVal >>= 8;
    output[2] = nVal;
...
static int rtcp_compute_roundtrip_time(Rtcp_t *rtcp,unsigned int last_sr_ts,
	unsigned int delay/*[MSB16:seconds,LSB16:fraction second*/)
{
	int rtt=0;
...
static void rtcp_compute_lost(Rtcp_t *rtcp,unsigned int srPs,unsigned int srOctect)
{
	unsigned int lost_ps;
	lost_ps = srPs -  rtcp->rtp->packet_cnt;
...
static int rtcp_handle_sr(Rtcp_t *rtcp,char *buf)
{
	...
	if(rtcp->src_id[0] == 0){ // withou ssrc (alaws would be init on RTSP-SETUP, TRANSPORT)
		rtcp->src_id[0] = rtcp->rtp->ssrc;
	}
	//if(ssrc != rtcp->src_id[0]){
	//	VLOG(VLOG_ERROR,"rtcp SR ssrc:%#08x invalid,expected:%#08x",ssrc,rtcp->src_id[0]);
...
static int rtcp_handle_rr(Rtcp_t *rtcp,char *buf)
{
	...
	unsigned int lost_cnt,max_seq,cycle;
	unsigned int delay_since_last_sr=0;
	if(rtcpHeader->version != RTCP_VERSION){
		VLOG(VLOG_WARNING,"rtcp version invalid");
	}
	...
	if(rtcpHeader->count > 0){
		...
		if(src_id != rtcp->src_id[0]){
			VLOG(VLOG_ERROR,"rtcp RR src_id:%#08x invalid,expected:%#08x",src_id,rtcp->src_id[0]);
			return RTSP_RET_FAIL;
		}
		...
		ptr += 3;
		cycle =(unsigned int) decode_int16be(ptr);
		ptr += 2;	
		...
		rtcp_compute_roundtrip_time(rtcp,rtcp->last_sr_ts,delay_since_last_sr);
	}
...
static int RTCP_send_packet(Rtcp_t *rtcp,int bBye)
{
	...
	//printf("total rtcp packets size:%d \n",size);
...
int RTCP_handle_packet(Rtcp_t *rtcp,void *payload,int payload_size)
{
	...
	if(rtcp->interleaved == true){
		...
		//
		ret=SOCK_recvfrom(rtcp->sock,rtcp->peername,&port,buf,sizeof(buf),0);
		if(ret == RTSP_RET_FAIL){
			return RTSP_RET_FAIL;
		}
		ptr = buf;
...
{
	int ret,size;
	char *pbuf = NULL,*ptr=NULL,*pend=NULL;
	rtcp_common_t *rtcpHeader=NULL;
	...
	if(rtcp->interleaved == true){
		...
		pbuf = rtcp->packet.trunk_buf;
		if (payload_size > sizeof(rtcp->packet.trunk_buf)) {
			printf("rtcp packet too big %d/%d\r\n", payload_size, sizeof(rtcp->packet.trunk_buf));
			return -1;
		}
		if (rtcp->packet.trunk_readed > 0) {
			//printf("rtcp has readed %d/%d\r\n", rtcp->packet.trunk_readed , payload_size);
			need_rcv -= rtcp->packet.trunk_readed;
			pbuf += rtcp->packet.trunk_readed;
		}

		//
		nret = SOCK_recv2(rtcp->sock, pbuf, need_rcv, 0); 
		if (nret < 0) {
			return -1;
		} else if (nret  != need_rcv) {
			 //printf("rtcp %s read %d/%d\r\n", rtcp->peername, nret, need_rcv);
			 rtcp->packet.trunk_readed += nret;
			return SOCK_EAGAIN;
		}

		ptr = rtcp->packet.trunk_buf;
		size  = payload_size;
		rtcp->packet.trunk_readed = 0;
		...
		if(rtcp->bAlive == false) return RTSP_RET_FAIL;
		//
		ret=SOCK_recvfrom(rtcp->sock,rtcp->peername,&portt,rtcp->packet.trunk_buf,sizeof(rtcp->packet.trunk_buf),0);
		if (ret == SOCK_EAGAIN) {
			return SOCK_EAGAIN;
		} else if (ret < 0) {
			return -1;
	...
	do{
		...
		VLOG(VLOG_DEBUG, "rtcp pakcket type:%d length:%d",rtcpHeader->pt,(ntohs(rtcpHeader->length)+1) * 4);
		if(ret == RTSP_RET_FAIL){
			VLOG(VLOG_ERROR,"RTCP BECOME DISABLE");
			rtcp->bAlive = false;
			return RTSP_RET_FAIL;
	...
	Rtp_t *rtp) 
{
	...
	*r = rtcp;
	...
	VLOG(VLOG_CRIT,"rtcp init done,sock:%d port_c:%d port_s:%d ssrc:%x",rtcp->sock,rtcp->chn_port_c,rtcp->chn_port_s, src_id);
	return RTSP_RET_OK;
}
