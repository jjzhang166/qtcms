...
int CSoupXml::OpenChannel(int nChannelId,int nStreamId,bool bOpen)
{
	...
	return 	m_ps->SendData(sOSDataXml,strlen(sOSDataXml)+1);
}
...
int CSoupXml::CheckUserMsg(char *sUsername,char *sPassword)
{
	...
	return 	m_ps->SendData(sCheckUserMsgXml,strlen(sCheckUserMsgXml)+1);
}
...
int  CSoupXml::GetStreamData(int nChannelId)
{
	...
	return 	m_ps->SendData(sGetStreamDataMsgXml,strlen(sGetStreamDataMsgXml)+1);
}
