...
long __stdcall BackUpActivity::QueryInterface( const IID & iid,void **ppv )
{
...
unsigned long __stdcall BackUpActivity::AddRef()
{
	m_csRef.lock();
	m_nRef ++;
	m_csRef.unlock();
...
unsigned long __stdcall BackUpActivity::Release()
{
	int nRet = 0;
	m_csRef.lock();
	m_nRef -- ;
...
void BackUpActivity::Active( QWebFrame * frame )
{
...
void BackUpActivity::OnTopActDbClick()
{
...
void BackUpActivity::OnCloseWindow()
{
	m_MainView->close();
}

void BackUpActivity::OnMaxsizeWindow()
{
	if (m_MainView->isMaximized()||m_MainView->size().height()==QApplication::desktop()->screenGeometry().height()||m_MainView->size().width()==QApplication::desktop()->screenGeometry().width())
	{
		m_MainView->showNormal();
...
void BackUpActivity::OnMinsizeWindow()
{
