...
{
	TurnServerInfo turn;
	QString tempip,tempport,tempserver;
	...
	//CloseTask(taskTemp);
	QDomElement turnserverElement = RootElement.nextSiblingElement("turnservers");
	turnserverElement = turnserverElement.firstChildElement("turnserver");
	tempserver = turnserverElement.childNodes().item(0).toText().data();
	//turn.ulDeviceAddr = inet_addr(dvripElement->GetText());
	turn.ulDeviceAddr = QHostAddress(tempip).toIPv4Address();
	//turn.ulDevicePort = atoi(dvrportElement->GetText());
	turn.ulDevicePort = tempport.toInt();
	...
	int ddindex = tempserver.indexOf(':');
	turn.ulServerAddr = QHostAddress(tempserver.left(ddindex)).toIPv4Address();

	/*char sPort[32] = {0};
	strncpy(sPort,pDivAddr + 1,strlen(sServerInfo) - strlen(sServerIp) - 1);
	turn.ulServerPort = atoi(sPort);*/
	turn.ulServerPort = tempserver.right(tempserver.length()-ddindex-1).toInt();
	return turn;
}
