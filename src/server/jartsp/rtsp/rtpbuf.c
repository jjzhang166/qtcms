...
int CircleAddRtpFrame(CircleBuffer_t* pCBuffer,RtpFrameInfo_t *info)
{
    ...
    if(iBytesToWrite)
    {
        ...
        pCBuffer->m_iWriteCursor += iBytesToWrite;
		if(pCBuffer->m_iWriteCursor >= pCBuffer->m_iBufferSize){
	        VLOG(VLOG_ERROR,"write cursor is overflow");
			exit(1);
		}
    }
...
CircleBuffer_t* CIRCLEBUFFER_new(const unsigned int iBufferSize,const int buffer_time)
{
	...
	pNewBuffer->NextFrame = CircleNextCompleteFrame;
	pNewBuffer->NextFrameTS = CircleNextFrameTimeStamp;
	pNewBuffer->AddRtpFrame = CircleAddRtpFrame;
	pNewBuffer->IsAvailable = CircleIsAvailable;
	
	pNewBuffer->Destroy = CircleDestroy;
	...
	VLOG(VLOG_CRIT,"create circle buffer(size:%u) success",iBufferSize);
    return pNewBuffer;
}
