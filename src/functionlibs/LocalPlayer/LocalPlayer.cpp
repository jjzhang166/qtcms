...
QStringList LocalPlayer::getFileList( int wndId, QDate date, QTime star, QTime end, int types, QVector<PeriodTime> &playTime )
{
	...
	QString sqlCommand = QString("select start_time, end_time, path from local_record where win_id='%1' and date='%2' and end_time>='%3' and start_time<='%4' and (%5) and start_time!=end_time order by start_time")
