	...
	char (*query_mac)[18], int out_size)
{
	...
	//printf("mac local:%s  query mac:%s ip:%s\n", src_mac, query_mac[0], query_ip);
        ...
        pkt.op = htons(type);
        get_hw_addr(pkt.targ_hw_addr, dst_mac);
        get_hw_addr(pkt.rcpt_hw_addr, query_mac[0]);
        get_hw_addr(pkt.src_hw_addr, src_mac);
        get_hw_addr(pkt.sndr_hw_addr, mac);
        get_ip_addr(&src_in_addr, ip);
        get_ip_addr(&targ_in_addr, query_ip);
...
*/
	...
	get_hw_addr(addr_ii.sll_addr, src_mac);
	...
	get_hw_addr(sa.sll_addr, dst_mac);
...
int ARP_send_request(char *query_ip, char (*query_mac)[18], int max_size, int timeout)
{
	...
	strncpy(sArpDefEther, getenv("DEF_ETHER"), sizeof(sArpDefEther));
	
	_get_if_addr(DEFAULT_DEVICE, ip, mac);
